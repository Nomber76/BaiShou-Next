...
const MatGridView = ({ color, size }: { color: string; size: number }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} fill={color}>
    <path d="M3 11h8V3H3v8zm0 10h8v-8H3v8zm10 0h8v-8h-8v8zm0-18v8h8V3h-8z" />
  </svg>
)
    ...
    <div className="dashboard-stats-card">
      ...
      <div className="stats-grid">
        <div className="stats-row">
          {renderStatTile(
            <MatBook size={22} color="#4CAF50" />,
            totalDiaryCount,
            t('summary.stats_daily', '日记总数'),
            'color-green'
          )}
          {renderStatTile(
            <MatViewWeek size={22} color="#3F51B5" />,
            totalWeeklyCount,
            t('summary.stats_weekly', '周统总数'),
            'color-indigo'
          )}
        </div>
        <div className="stats-row">
          {renderStatTile(
            <MatGridView size={22} color="#2196F3" />,
            totalMonthlyCount,
            t('summary.stats_monthly', '月统总数'),
            'color-blue'
          )}
          {renderStatTile(
            <MatDateRange size={22} color="#FBC02D" />,
            totalQuarterlyCount,
            t('summary.stats_quarterly', '季统总数'),
            'color-amber'
          )}
        </div>
        <div className="stats-row full">
          {renderStatTile(
            <MatCalendarToday size={22} color="#FF9800" />,
            totalYearlyCount,
            t('summary.stats_yearly', '年统总数'),
            'color-orange'
          )}
        </div>
      </div>
